...
var framebuffer: [width * pages]u8 align(32) = [_]u8{0} ** (width * pages);

pub fn Display(comptime BusType: type) type {
    return struct {
        const Self = @This();
        ...
        pub fn command(self: Self, byte: u8) bool {
            return self.commands(&[_]u8{byte});
        }

        pub fn commands(self: Self, bytes: []const u8) bool {
            return self.bus.writeRegisterBytes(self.addr, 0x00, bytes);
        }

        pub fn fill(self: Self, pattern: u8) void {
            for (&framebuffer) |*byte| {
                byte.* = pattern;
            }
            self.writeFrame();
        }

        pub fn drawTwoColorTestText(self: Self) void {
            const line1 = "ZIG ON";
            const line2 = "HPM5301";
            const scale: u16 = 2;
        ...
        pub fn drawTextFrame(
            ...
            while (page < pages) : (page += 1) {
                ...
                while (x < width) : (x += 1) {
                    ...
                    framebuffer[page * width + x] = value;
                }
            }
        ...
        fn writeFrame(self: Self) void {
            self.setFullWindow();
            _ = self.bus.writeRegisterBytes(self.addr, 0x40, framebuffer[0..]);
        }

        fn setFullWindow(self: Self) void {
